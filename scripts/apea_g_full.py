...
def main() -> int:
    ...
    if not approved: print(json.dumps({"status": "AWAITING_PLAN_APPROVAL", "capabilities": len(plan["capabilities"])})); return 0
    ...
    for item in plan["capabilities"]:
        ...
        if not capability_plan["steps"]: state.setdefault("completed_capabilities", []).append(capability); continue
        state["capability"] = capability
        core.execute_capability(capability_plan, state, branch)
        state.setdefault("completed_capabilities", []).append(capability); state["completed_capabilities"] = list(dict.fromkeys(state["completed_capabilities"]))
        state["completed"] = []; state["status"] = "advancing"; save(STATE_PATH, state)
        core.sh("git", "add", "--", ".")
        if core.sh("git", "diff", "--cached", "--quiet") != "":
            core.sh("git", "config", "user.name", "APEA-G"); core.sh("git", "config", "user.email", "apea-g@users.noreply.github.com")
            core.sh("git", "commit", "-m", f"chore: persist APEA-G {capability} progress", check=True); core.sh("git", "push", "-u", "origin", f"HEAD:{branch}", check=True)
