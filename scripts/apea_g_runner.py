"""Continuous APEA-G entrypoint: execute the successor loop automatically."""
...
def bootstrap_loop_state() -> None:
    ...
    if not isinstance(state.get("loop"), dict):
        ...
        state["loop"] = {"plan_id": successor.get("plan_id", "apea-g-loop-v1"), "cursor": 0, "current_capability": unfinished[0] if unfinished else None, "status": "ready" if unfinished else "complete", "completed_capabilities": []}
        state["history"].append({"action": "loop-bootstrap", "plan_id": state["loop"]["plan_id"], "current_capability": state["loop"]["current_capability"]})
...
def reconcile_ci_state() -> None:
    ...
    if not event_path or not Path(event_path).exists() or not STATE_PATH.exists():
        return
    ...
    if state.get("step_status") in {"in_progress", "ci_pending", "repairing"}:
        decision = decide(ci_conclusion=conclusion, step_status="ci_pending", repair_attempts=int(state.get("repair_attempts", 0)))
        state["last_ci_conclusion"] = conclusion
        state["last_ci_run_id"] = run.get("id")
        if decision.action is GateAction.ADVANCE:
            state["step_status"] = "complete"
        elif decision.action is GateAction.REPAIR:
            state["step_status"] = "repairing"
        else:
