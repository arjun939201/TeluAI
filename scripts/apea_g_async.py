...
def ensure_plan(state):
 ...
 if not plan or plan.get('capability')!=cap: plan=core.make_capability_plan(cap)
 return plan
def commit_step(plan,state,branch,index,step,repair=False):
 ...
 result=core.provider(prompt); 
 if result.get('action')=='blocked': raise RuntimeError(result.get('reason','step blocked'))
 core.apply_patch(result.get('patch')); core.validate_local()
...
def resume(state,branch):
 ...
 repairs+=1; result=core.repair(plan,step,evidence,core.render_context(capability=cap,failure_signature=failure.get('signature')))
 if not result.get('patch'): raise RuntimeError(result.get('diagnosis','repair blocked'))
 core.apply_patch(result['patch']); core.validate_local(); state['pending']['repairs']=repairs; state['pending']['failure_signature']=failure.get('signature'); state['status']='awaiting_ci'; save(STATE_PATH,state)
