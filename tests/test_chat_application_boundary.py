...
def test_chat_preparation_uses_single_application_boundary(monkeypatch):
    ...
    def fake_prepare_prompt(message, mode, history, user_id, response_length="normal"):
        class Decision: use_melimi=False; language="telugu"
    ...
    assert isinstance(turn,PreparedChatTurn); assert turn.conversation_id=="conversation-1"; assert turn.prompt=="canonical prompt"; assert turn.metadata["intent"]=="conversation"
