...
@pytest.mark.asyncio
async def test_json_chat_runtime_uses_canonical_prepared_turn(monkeypatch):
...
@pytest.mark.asyncio
async def test_stream_chat_runtime_uses_canonical_prepared_turn(monkeypatch):
    monkeypatch.setattr(middleware, "get_llm_provider", lambda: _Provider())
    monkeypatch.setattr(middleware, "direct_lookup", lambda message: None)
    monkeypatch.setattr(middleware, "clean_response", lambda text: text)
    monkeypatch.setattr(middleware, "append_user_message", lambda *args, **kwargs: None)
    ...
    events = [json.loads(chunk.decode().removeprefix("data: ").strip()) async for chunk in response.body_iterator]
