...
def _cache_key(message: str, mode: str, history=None, response_length: str = "normal") -> str:
    ...
    material = f"chat-conversation-v5\n{mode}\n{response_length}\n{_knowledge_version()}\n{context_hash}\n{normalized}"
...
def _language_command_result(message: str, user) -> tuple[str, str]:
    parsed = parse_command(message)
    if not parsed:
        raise ValueError("Invalid language command.")
    kind, payload = parsed
    if user.role in {"admin", "owner"}:
        result = learn_explicit_teaching(message, user.id)
        if not result.get("learned"):
            raise ValueError("Invalid language command.")
        return "MASTER", "✓ మేలిమి భాషా నిలయంలో నేరుగా చేర్చబడింది.\nస్థితి: MASTER"
    submission = submit_command_candidate(kind, payload, message, user.id)
    return "PENDING", ("✓ మీ భాషా చేర్పు సమీక్షకు పంపబడింది.\n" f"స్థితి: PENDING\nచేర్పు సంఖ్య: {submission.candidate_id}")
...
def admin_learning_approve(candidate_id: int, note: str = "", user=Depends(require_admin)):
    result = review_learning_candidate(candidate_id, True, note, user.id)
...
def admin_learning_reject(candidate_id: int, note: str = "", user=Depends(require_admin)):
    result = review_learning_candidate(candidate_id, False, note, user.id)
    if result is None: raise HTTPException(404, "Candidate not found.")
...
def admin_database_language(limit: int = 50, user=Depends(require_admin)):
    return {"language": language_snapshot(max(1, min(limit, 500))), "role": user.role}
...
@app.get("/admin/database/lexicon")
def admin_database_lexicon(word: str, user=Depends(require_admin)):
    """Inspect the deterministic lexical layer without invoking an LLM."""
    return {"lookup": lookup_word(word), "analysis": analyze_word(word), "role": user.role}
...
def admin_set_active(target_id: int, active: bool, user=Depends(require_owner)):
    ...
    audit_log(user.id, "user.active_change", "user", str(target_id), {"active": bool(active)})
    return {"ok": True, "user": result}
...
def auth_guest(payload: GuestRegisterRequest, response: Response):
    try:
        ...
        _set_session_cookie(response, create_session(user.id, settings.session_days))
...
def auth_bootstrap_owner(user=Depends(current_user)):
    ...
    if not configured: raise HTTPException(503, "Owner bootstrap is not configured.")
    if not user.email or user.email.lower() != configured: raise HTTPException(403, "This account is not the configured owner account.")
...
def auth_reset_password(payload: ResetPasswordRequest, response: Response):
    ...
    _set_session_cookie(response, create_session(user_id, settings.session_days)); audit_log(user_id, "auth.password_reset", "user", str(user_id)); return {"ok": True}
...
@app.post("/feedback")
def submit_feedback(payload: FeedbackRequest, user=Depends(current_user)):
    with SessionLocal() as db:
        if payload.message_id is not None:
            from app.database import Message
            message = db.get(Message, payload.message_id)
            if message is None or message.user_id != user.id: raise HTTPException(404, "Message not found.")
        row = Feedback(user_id=user.id, message_id=payload.message_id, rating=payload.rating, text=payload.text.strip()); db.add(row); db.commit(); db.refresh(row)
    audit_log(user.id, "feedback.create", "feedback", str(row.id), {"rating": payload.rating}); return {"ok": True, "id": row.id}
...
async def github_sync(user=Depends(require_admin)):
    ...
    try: result = await sync_repo()
    except GitHubSyncError as exc: raise HTTPException(502, str(exc)) from exc
    audit_log(user.id, "github.sync", "repository", result.get("repository", ""), result); return result
...
async def melimi_content_upload(file: UploadFile = File(...), user=Depends(current_user)):
    ...
    approved = user.role in {"admin", "owner"}
    try: result = ingest_language_package(filename=name, raw=raw, approved=approved, actor_user_id=user.id)
    except ValueError as exc: raise HTTPException(400, str(exc)) from exc
...
async def melimi_content(payload: dict, user=Depends(current_user)):
    ...
    audit_log(user.id, "language.content_create", "language_content", str(result.get("candidate_id", result.get("id", ""))), {"title": title, "status": result.get("status")}); return {"ok": True, **result}
...
async def chat(request: ChatRequest, user=Depends(current_user)):
    ...
    if command:
        try: status, reply = _language_command_result(message, user)
        except ValueError as exc: raise HTTPException(400, str(exc)) from exc
        conversation_id = request.conversation_id or create_conversation(user.id, message[:70], request.mode); save_message(user.id, conversation_id, "user", message); assistant_id = save_message(user.id, conversation_id, "assistant", reply, model="language-command")
    ...
    if request.conversation_id:
        ...
        history = [turn.model_dump() for turn in request.history[-settings.max_history_turns :]]; conversation_id = create_conversation(user.id, message[:70], request.mode)
    user_settings = get_user_settings(user.id); persistent = recall_user_memory(user.id) if user_settings.get("memory_enabled", True) else []; cacheable = not request.conversation_id and not persistent
    if settings.cache_enabled and cacheable:
        cached = cache_get(_cache_key(message, request.mode, history, user_settings.get("response_length", "normal")), request.mode)
        if cached:
    ...
    if local is not None:
        save_message(user.id, conversation_id, "user", message); aid = save_message(user.id, conversation_id, "assistant", local, model="local-deterministic"); return ChatResponse(reply=local, mode=request.mode, intent="local_knowledge", conversation_id=conversation_id, message_id=aid, local=True)
    state = from_history(history); linguistic = extract_linguistic_hints(message); input_info = analyze_input(message); understanding = infer_intent(message, state); conversation = build_context(message, state, linguistic); plan = plan_response(understanding); memory = format_memory(extract_memory_candidates(history, settings.max_memory_items))
    ...
    prompt = build_prompt(mode=request.mode, conversation=conversation, linguistics=linguistic_text, memory=memory, grammar=grammar_policy() if request.mode == "melimi" else "", plan=plan, melimi_engine=melimi_engine, knowledge="")
    ...
    save_usage(user.id, result.get("model"), result.get("input_tokens"), result.get("output_tokens"), "ok"); save_message(user.id, conversation_id, "user", message)
    aid = save_message(user.id, conversation_id, "assistant", reply, model=result.get("model"), input_tokens=result.get("input_tokens"), output_tokens=result.get("output_tokens"), latency_ms=result.get("latency_ms"))
    if settings.cache_enabled and cacheable and len(reply) >= settings.cache_min_chars: cache_put(_cache_key(message, request.mode, history, user_settings.get("response_length", "normal")), request.mode, reply)
