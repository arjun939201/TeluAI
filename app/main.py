...
from app.database import (
    ...
    get_user_by_id,
    get_user_settings,
    ingest_language_package,
    language_snapshot,
    list_audit_logs,
    list_candidates,
    list_users,
    promote_configured_owners,
    recall_user_memory,
...
app = FastAPI(
    title="TeluAI — Melimi Telugu AI Platform",
...
def _language_command_result(message: str, user) -> tuple[str, str]:
    ...
    if user.role in {"admin", "owner"}:
        ...
        if not result.get("learned"):
            raise ValueError("Invalid language command.")
        return "MASTER", f"✓ మేలిమి భాషా నిలయంలో నేరుగా చేర్చబడింది.\nస్థితి: MASTER"
...
def auth_bootstrap_owner(user=Depends(current_user)):
    ...
    if not user.email or user.email.lower() != configured:
...
def auth_reset_password(payload: ResetPasswordRequest, response: Response):
    ...
    _set_session_cookie(response, create_session(user_id, settings.session_days))
    audit_log(user_id, "auth.password_reset", "user", str(user_id))
    return {"ok": True}
...
def submit_feedback(payload: FeedbackRequest, user=Depends(current_user)):
    with SessionLocal() as db:
        ...
        row = Feedback(user_id=user.id, message_id=payload.message_id, rating=payload.rating, text=payload.text.strip())
        db.add(row)
        db.commit()
        db.refresh(row)
    audit_log(user.id, "feedback.create", "feedback", str(row.id), {"rating": payload.rating})
...
async def melimi_content_upload(file: UploadFile = File(...), user=Depends(current_user)):
    ...
    try:
        result = ingest_language_package(filename=name, raw=raw, approved=approved, actor_user_id=user.id)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
...
async def melimi_content(payload: dict, user=Depends(current_user)):
    ...
    try:
        result = submit_content(title=title, content=content, meaning=meaning, user_id=user.id)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    audit_log(user.id, "language.content_create", "language_content", str(result.get("candidate_id", result.get("id", ""))), {"title": title, "status": result.get("status")})
    return {"ok": True, **result}
...
async def chat(request: ChatRequest, user=Depends(current_user)):
    ...
    if command:
        try:
            status, reply = _language_command_result(message, user)
        except ValueError as exc:
            raise HTTPException(400, str(exc)) from exc
    ...
    if request.conversation_id:
        try:
            history = [{"role": x["role"], "content": x["content"]} for x in get_history(user.id, request.conversation_id, limit=40)]
        except ValueError as exc:
            raise HTTPException(404, str(exc)) from exc
        conversation_id = request.conversation_id
    else:
        history = [turn.model_dump() for turn in request.history[-settings.max_history_turns :]]
        conversation_id = create_conversation(user.id, message[:70], request.mode)

    user_settings = get_user_settings(user.id)
    persistent = recall_user_memory(user.id) if user_settings.get("memory_enabled", True) else []
    cacheable = not request.conversation_id and not persistent
    if settings.cache_enabled and cacheable:
        ...
        if cached:
            save_message(user.id, conversation_id, "user", message)
            aid = save_message(user.id, conversation_id, "assistant", cached, model="cache")
            return ChatResponse(reply=cached, mode=request.mode, intent="cached", conversation_id=conversation_id, message_id=aid, local=True)

    local = local_answer(message, request.mode)
    if local is not None:
        save_message(user.id, conversation_id, "user", message)
        aid = save_message(user.id, conversation_id, "assistant", local, model="local-deterministic")
    ...
    try:
        ...
        if request.mode == "melimi":
            ...
            if converted != normalized and len(normalized) <= 500:
                save_message(user.id, conversation_id, "user", message)
                aid = save_message(user.id, conversation_id, "assistant", converted, model="local-melimi-fallback")
                return ChatResponse(reply=converted, mode=request.mode, intent="local_conversion_fallback", conversation_id=conversation_id, message_id=aid, local=True)
        raise HTTPException(502, str(exc)) from exc
    except Exception as exc:
        save_usage(user.id, settings.groq_model, None, None, "error")
    ...
    if settings.cache_enabled and cacheable and len(reply) >= settings.cache_min_chars:
        cache_put(_cache_key(message, request.mode, history, user_settings.get("response_length", "normal")), request.mode, reply)
    return ChatResponse(
        reply=reply,
        mode=request.mode,
