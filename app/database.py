...
def _upgrade_existing_schema():
    ...
    additions = {"melimi_roots": {"version":"INTEGER DEFAULT 1", "updated_at":"DATETIME"}, "melimi_documents": {"version":"INTEGER DEFAULT 1"}, "conversations": {"summary":"TEXT DEFAULT ''"}, "users": {"role":"TEXT DEFAULT 'user'", "is_active":"BOOLEAN DEFAULT TRUE"}}
    with engine.begin() as conn:
        for table, cols in additions.items():
            if not inspector.has_table(table): continue
            existing={c["name"] for c in inspect(engine).get_columns(table)}
            for name, sqltype in cols.items():
                if name not in existing: conn.exec_driver_sql(f"ALTER TABLE {table} ADD COLUMN {name} {sqltype}")

def init_db():
    Base.metadata.create_all(engine); _upgrade_existing_schema(); Base.metadata.create_all(engine); _seed_language()
...
def create_password_reset_token(email):
    code = f"{secrets.randbelow(1_000_000):06d}"; h = hashlib.sha256(code.encode()).hexdigest()
    with SessionLocal() as db:
        user = db.scalar(select(User).where(User.email == email.lower()))
        if not user or not getattr(user, "is_active", True): return None
        db.execute(delete(PasswordResetToken).where(PasswordResetToken.user_id == user.id)); db.add(PasswordResetToken(token_hash=h,user_id=user.id,expires_at=now()+timedelta(minutes=10))); db.commit()
    return code
...
def verify_password_reset_code(email, code):
    ...
    with SessionLocal() as db:
        user = db.scalar(select(User).where(User.email == email.lower()))
        if not user or not getattr(user, "is_active", True): return None
        row = db.scalar(select(PasswordResetToken).where((PasswordResetToken.user_id == user.id)&(PasswordResetToken.token_hash == code_hash)))
        if not row or row.used_at is not None or row.expires_at < now(): return None
        reset_token = secrets.token_urlsafe(48); reset_hash = hashlib.sha256(reset_token.encode()).hexdigest(); db.delete(row); db.flush(); db.add(PasswordResetToken(token_hash=reset_hash,user_id=user.id,expires_at=now()+timedelta(minutes=10))); db.commit(); return reset_token

def reset_password(raw_token, new_password):
    if not raw_token or len(new_password) < 8: return None
    ...
    with SessionLocal() as db:
        ...
        if not user or not getattr(user, "is_active", True): return None
        user.password_hash = _hash_password(new_password); row.used_at = now(); db.execute(delete(Session).where(Session.user_id == user.id)); db.commit(); return user.id
...
def approved_learning():
    with SessionLocal() as db:
        rows=db.scalars(select(LearningCandidate).where(LearningCandidate.status=="APPROVED").order_by(LearningCandidate.created_at.asc())).all(); return [json.loads(r.payload_json or "{}")|{"knowledge_type":r.knowledge_type} for r in rows]
...
def _upsert_language_json(db, seed: dict, source_name: str):
    ...
    for item in seed.get("roots", []):
        ...
        if not source or not target: continue
        row=db.scalar(select(MelimiRoot).where(MelimiRoot.standard_root==source))
        if row: row.melimi_root=target; row.meaning=str(item.get("meaning",row.meaning)); row.category=str(item.get("category",row.category)); row.status="MASTER"; row.source=source_name; row.version+=1
        else: db.add(MelimiRoot(standard_root=source,melimi_root=target,meaning=str(item.get("meaning","")),category=str(item.get("category","")),status="MASTER",source=source_name))
        counts["roots"]+=1
    for doc in seed.get("documents", []):
        path=str(doc.get("path","")).strip()
        if not path: continue
        row=db.scalar(select(MelimiDocument).where(MelimiDocument.path==path)); payload=json.dumps(doc.get("entries",[]),ensure_ascii=False)
        if row: row.kind=str(doc.get("kind",row.kind)); row.text=str(doc.get("text",row.text)); row.entries_json=payload; row.source=source_name; row.status="MASTER"; row.version+=1
        else: db.add(MelimiDocument(path=path,kind=str(doc.get("kind","other")),text=str(doc.get("text","")),entries_json=payload,source=source_name,status="MASTER"))
        counts["documents"]+=1
    for item in seed.get("affixes", []):
        form=str(item.get("form","")).strip(); kind=str(item.get("kind","other")).strip()
        if not form: continue
        row=db.scalar(select(MelimiAffix).where((MelimiAffix.form==form)&(MelimiAffix.kind==kind)))
        if row: row.meaning=str(item.get("meaning",row.meaning)); row.applies_to=str(item.get("applies_to",row.applies_to)); row.notes=str(item.get("notes",row.notes)); row.status="MASTER"; row.source=source_name
        else: db.add(MelimiAffix(form=form,kind=kind,meaning=str(item.get("meaning","")),applies_to=str(item.get("applies_to","")),notes=str(item.get("notes","")),status="MASTER",source=source_name))
        counts["affixes"]+=1
    for item in seed.get("rules", []):
        name=str(item.get("name","")).strip()
        if not name: continue
        row=db.scalar(select(MelimiRule).where(MelimiRule.name==name))
        if row: row.category=str(item.get("category",row.category)); row.rule_text=str(item.get("rule_text",row.rule_text)); row.operation=str(item.get("operation",row.operation)); row.status="MASTER"; row.source=source_name; row.version+=1
        else: db.add(MelimiRule(name=name,category=str(item.get("category","grammar")),rule_text=str(item.get("rule_text","")),operation=str(item.get("operation","")),status="MASTER",source=source_name))
        counts["rules"]+=1
    for item in seed.get("examples", []): db.add(MelimiExample(standard_text=str(item.get("standard","")),melimi_text=str(item.get("melimi","")),category=str(item.get("category","")),source=source_name,status="MASTER")); counts["examples"]+=1
    for item in seed.get("knowledge", []):
        key=str(item.get("key","")).strip()
        if not key: continue
        kind=str(item.get("kind","FACT")); row=db.scalar(select(KnowledgeEntry).where((KnowledgeEntry.kind==kind)&(KnowledgeEntry.key==key)))
        if row: row.value=str(item.get("value",row.value)); row.metadata_json=json.dumps(item.get("metadata",{}),ensure_ascii=False); row.status="MASTER"; row.source=source_name; row.version+=1
        else: db.add(KnowledgeEntry(kind=kind,key=key,value=str(item.get("value","")),metadata_json=json.dumps(item.get("metadata",{}),ensure_ascii=False),status="MASTER",source=source_name))
        counts["knowledge"]+=1
    return counts

def ingest_language_package(filename: str, raw: bytes, approved: bool, actor_user_id: int | None = None):
    ext=os.path.splitext(filename.lower())[1]; files={}
    if ext==".zip":
        try:
            with zipfile.ZipFile(io.BytesIO(raw)) as z:
                for info in z.infolist():
                    if info.is_dir(): continue
                    inner=os.path.basename(info.filename); iext=os.path.splitext(inner.lower())[1]
    ...
    else: files[os.path.basename(filename) or "language_content"+ext]=raw
    with SessionLocal() as db:
        counts={"roots":0,"documents":0,"affixes":0,"rules":0,"examples":0,"knowledge":0,"files":0}
        if approved:
            for name,data in files.items():
                ext2=os.path.splitext(name.lower())[1]; text=data.decode("utf-8-sig","replace")
                if ext2==".json":
                    ...
                    path=f"uploads/{filename}/{name}"; row=db.scalar(select(MelimiDocument).where(MelimiDocument.path==path))
                    if row: row.text=text; row.kind="uploaded_content"; row.source=f"upload:{filename}"; row.status="MASTER"; row.version+=1
                    else: db.add(MelimiDocument(path=path,kind="uploaded_content",text=text,entries_json="[]",source=f"upload:{filename}",status="MASTER"))
                    counts["documents"]+=1
                counts["files"]+=1
            db.add(KnowledgeVersion(version=knowledge_version()+1,source=f"upload:{filename}",checksum=hashlib.sha256(raw).hexdigest())); db.commit(); return {"status":"APPROVED","files":counts["files"],"documents":counts["documents"],"counts":counts}
        payload={"filename":filename,"bytes":len(raw),"files":[]}
        for name,data in files.items(): payload["files"].append({"name":name,"content":data.decode("utf-8-sig","replace")[:500000]})
...
def register_language_root(standard_root,melimi_root,metadata=None): return _register_language_root_approved({"standard_root":standard_root.strip(),"melimi_root":melimi_root.strip(),**(metadata or {})})
...
def get_user_by_id(user_id: int):
    with SessionLocal() as db: return db.get(User, user_id)

def list_users():
    with SessionLocal() as db:
        rows = db.scalars(select(User).order_by(User.created_at.asc())).all(); return [{"id":r.id,"username":r.username,"email":r.email,"role":r.role,"is_active":r.is_active,"created_at":r.created_at.isoformat(),"last_login":r.last_login.isoformat() if r.last_login else None} for r in rows]

def set_user_role(target_id: int, role: str):
    role=role.lower().strip()
    if role not in {"user","admin","owner"}: raise ValueError("Invalid role.")
    with SessionLocal() as db:
        row=db.get(User,target_id)
        if not row:return None
        row.role=role; db.commit(); db.refresh(row); return {"id":row.id,"username":row.username,"email":row.email,"role":row.role,"is_active":row.is_active}

def set_user_active(target_id: int, active: bool):
    with SessionLocal() as db:
        row=db.get(User,target_id)
        if not row:return None
        row.is_active=bool(active); db.commit(); db.refresh(row); return {"id":row.id,"username":row.username,"email":row.email,"role":row.role,"is_active":row.is_active}
...
def database_stats():
    with SessionLocal() as db:
        return {"users":db.scalar(select(func.count()).select_from(User)) or 0,"active_users":db.scalar(select(func.count()).select_from(User).where(User.is_active.is_(True))) or 0,"admins":db.scalar(select(func.count()).select_from(User).where(User.role=="admin")) or 0,"owners":db.scalar(select(func.count()).select_from(User).where(User.role=="owner")) or 0,"conversations":db.scalar(select(func.count()).select_from(Conversation)) or 0,"messages":db.scalar(select(func.count()).select_from(Message)) or 0,"melimi_roots":db.scalar(select(func.count()).select_from(MelimiRoot)) or 0,"melimi_documents":db.scalar(select(func.count()).select_from(MelimiDocument)) or 0,"melimi_affixes":db.scalar(select(func.count()).select_from(MelimiAffix)) or 0,"melimi_rules":db.scalar(select(func.count()).select_from(MelimiRule)) or 0,"melimi_examples":db.scalar(select(func.count()).select_from(MelimiExample)) or 0,"knowledge_entries":db.scalar(select(func.count()).select_from(KnowledgeEntry)) or 0,"pending_learning":db.scalar(select(func.count()).select_from(LearningCandidate).where(LearningCandidate.status=="PENDING")) or 0,"feedback":db.scalar(select(func.count()).select_from(Feedback)) or 0,"usage_records":db.scalar(select(func.count()).select_from(Usage)) or 0,"audit_logs":db.scalar(select(func.count()).select_from(AuditLog)) or 0}

def list_audit_logs(limit: int = 100):
    with SessionLocal() as db:
        rows=db.scalars(select(AuditLog).order_by(AuditLog.created_at.desc()).limit(max(1,min(limit,500)))).all(); return [{"id":r.id,"actor_user_id":r.actor_user_id,"action":r.action,"target_type":r.target_type,"target_id":r.target_id,"details":json.loads(r.details_json or "{}"),"created_at":r.created_at.isoformat()} for r in rows]

def promote_configured_owners(emails=None):
    configured=emails if emails is not None else [e.strip().lower() for e in os.getenv("OWNER_EMAILS","").split(",") if e.strip()]
    if not configured:return []
    promoted=[]
    with SessionLocal() as db:
        for email in configured:
            row=db.scalar(select(User).where(User.email==email))
            if row and row.role!="owner": row.role="owner"; promoted.append({"id":row.id,"email":row.email,"role":"owner"})
...
def bootstrap_owner(email: str):
    ...
    with SessionLocal() as db:
        ...
        if existing_owner:return None,"An owner already exists."
        row=db.scalar(select(User).where(User.email==email))
        if not row:return None,"Register the owner account first using the configured owner email."
        row.role="owner"; db.commit(); db.refresh(row); return row,None

def language_snapshot(limit: int = 50):
    with SessionLocal() as db:
        roots=db.scalars(select(MelimiRoot).order_by(MelimiRoot.updated_at.desc()).limit(max(1,min(limit,200)))).all(); rules=db.scalars(select(MelimiRule).order_by(MelimiRule.id.desc()).limit(max(1,min(limit,200)))).all(); return {"roots":[{"id":r.id,"standard_root":r.standard_root,"melimi_root":r.melimi_root,"meaning":r.meaning,"status":r.status,"source":r.source,"version":r.version} for r in roots],"rules":[{"id":r.id,"name":r.name,"category":r.category,"status":r.status,"source":r.source,"version":r.version} for r in rules]}
