...
def load_vocabulary(version: int | None = None) -> List[Dict[str, Any]]:
    try:
        ...
        for doc in language_documents():
            for entry in doc.get("entries",[]):
                if isinstance(entry,dict):
                    item=dict(entry); item.setdefault("_source",doc.get("path","")); item.setdefault("_kind",doc.get("kind","other")); out.append(item)
        return out
    except Exception:
        return []
...
def _tokens(text:str)->set[str]: return set(re.findall(r"[\u0C00-\u0C7F]+|[A-Za-z]+",norm(text)))
def retrieve(text:str,limit:int=24)->List[Dict]:
    ...
    for index,entry in enumerate(load_vocabulary()):
        ...
        overlap=sum(1 for word in qwords if len(word)>=2 and word in searchable); score += min(overlap,8)*8
...
def format_knowledge(entries:List[Dict],max_chars:int=6000)->str:
    ...
    for entry in entries:
        standard=str(entry.get("standard","")).strip(); melimi=str(entry.get("melimi","")).strip(); content=str(entry.get("content","")).strip(); note=str(entry.get("note","")).strip()
        if content:
