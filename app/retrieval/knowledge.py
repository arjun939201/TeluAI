...
def _load_vocabulary_for_version(version: int) -> List[Dict[str, Any]]:
    try:
...
def load_vocabulary() -> List[Dict[str, Any]]:
    try:
        ...
        return _load_vocabulary_for_version(language_space_version())
    except Exception:
        return []

load_vocabulary.cache_clear = _load_vocabulary_for_version.cache_clear  # type: ignore[attr-defined]

def reload_vocabulary():
    _load_vocabulary_for_version.cache_clear()
