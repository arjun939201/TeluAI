...
def reload_vocabulary(): load_vocabulary.cache_clear()

def fields(entry:Dict)->str:
    values=[]
    for key in ("standard","melimi","note","meaning","definition","english","gloss","description","example","examples","category","tags","related","synonyms"):
        value=entry.get(key,"")
...
def retrieve(text:str,limit:int=24)->List[Dict]:
    query=norm(text)
    if not query:return []
    qwords=set(re.findall(r"[\u0C00-\u0C7F]+|[A-Za-z]+",query)); scored=[]
    for index,entry in enumerate(load_vocabulary()):
        standard=norm(entry.get("standard","")); melimi=norm(entry.get("melimi","")); searchable=fields(entry); score=0
        if standard and standard in query: score+=300+len(standard)
        if melimi and melimi in query: score+=350+len(melimi)
        for word in qwords:
            if len(word)>=2 and word in searchable: score+=5
...
def format_knowledge(entries:List[Dict],max_chars:int=6000)->str:
    ...
    for entry in entries:
        standard=str(entry.get("standard","")).strip(); melimi=str(entry.get("melimi","")).strip(); note=str(entry.get("note","")).strip()
        if not standard and not melimi: continue
