...
def parse_command(message: str):
    """Parse explicit language insertion commands; ordinary chat never matches."""
    ...
    if raw_kind in {"word", "meaning", "correct"}:
        ...
        source, melimi = parsed.group("source").strip(), parsed.group("melimi").strip()
        ...
        if kind == "word":
            ...
            if row:
                if row.melimi_root != melimi_root or row.status != "MASTER" or row.source != source:
                    row.standard_root, row.melimi_root, row.status, row.source = standard, melimi_root, "MASTER", source
                    row.version += 1; row.updated_at = now(); changed = True
            else:
                db.add(MelimiRoot(standard_root=standard, melimi_root=melimi_root, status="MASTER", source=source)); changed = True
            changed = _upsert_knowledge(db, "VOCABULARY", f"word:{standard.casefold()}", melimi, {"standard": standard, "melimi": melimi, "command": payload.get("command", "word")}, source) or changed
            roots = 1
        else:
            content, meaning, command = payload["content"], payload.get("meaning", ""), payload.get("command", "content")
            target_kind = {"example": "EXAMPLE", "phrase": "PHRASE", "note": "NOTE", "content": "CONTENT", "root": "ROOT", "affix": "AFFIX", "rule": "RULE"}.get(command, "CONTENT")
            key = f"chat:{command}:{hashlib.sha256((content + '\n' + meaning).encode()).hexdigest()}"
            changed = _upsert_knowledge(db, target_kind, key, content, {"meaning": meaning, "command": command, "source": "chat-command"}, source) or changed
            if command in {"example", "content", "phrase"}:
                ...
                if row:
                    if row.meaning != meaning or row.status != "MASTER":
                        row.meaning, row.status, row.source = meaning, "MASTER", source; row.version += 1; row.updated_at = now(); changed = True
                else:
                ...
                if existing:
                    if existing.meaning != meaning or existing.status != "MASTER":
                        existing.meaning, existing.status, existing.source = meaning, "MASTER", source; changed = True
                else:
                ...
                if existing:
                    if existing.rule_text != meaning or existing.status != "MASTER":
                        existing.rule_text, existing.status, existing.source = meaning, "MASTER", source; existing.version += 1; changed = True
                else:
                    db.add(MelimiRule(name=content, category="chat-command", rule_text=meaning, status="MASTER", source=source)); changed = True
        if changed:
...
def reload_indexes():
    """Reload in-process language indexes without touching conversations."""
