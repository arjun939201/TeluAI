...
CURRENT_USER_ID: ContextVar[int | None] = ContextVar("teluai_learning_user_id", default=None)
CURRENT_USER_ROLE: ContextVar[str] = ContextVar("teluai_learning_user_role", default="user")

_TELUGU_RE = re.compile(r"[\u0C00-\u0C7F]")
_MAPPING_RE = re.compile(r"(?<!\w)([^=→➜⇒\n]{1,100}?)(?:\s*=\s*|\s*(?:→|➜|⇒|->)\s*)([^=→➜⇒\n]{1,120})(?=$|[.!?;\n])")


def set_request_user(user_id: int | None, role: str = "user"):
    return CURRENT_USER_ID.set(user_id), CURRENT_USER_ROLE.set(str(role or "user").lower())


def reset_request_user(tokens) -> None:
    if tokens:
        CURRENT_USER_ID.reset(tokens[0]); CURRENT_USER_ROLE.reset(tokens[1])
...
def _mapping_pairs(message: str) -> list[tuple[str, str]]:
    ...
    for pair in found:
        ...
        if key not in seen:
            seen.add(key); result.append(pair)
    return result


def record_chat_learning(user_id: int, role: str, message: str) -> int:
    """Persist explicit Melimi suggestions in the correct scope."""
...
def search_learning(query: str, user_id: int, limit: int = 8) -> list[dict[str, Any]]:
    ...
    with engine.begin() as conn:
        ...
        """), {"global_scope": "global", "user_scope": f"user:{int(user_id)}"}).mappings().all()
    hits = []
    for row in rows:
        ...
        if score:
            ...
            item["_score"] = score; hits.append(item)
...
def exact_mapping(word: str, user_id: int) -> str | None:
    """Resolve global or this user's private taught mapping, never another user's."""
    _init()
    with engine.begin() as conn:
        row = conn.execute(text("""
            SELECT standard, melimi FROM melimi_chat_learning
            WHERE kind='vocabulary' AND standard=:word
              AND scope IN ('global', :user_scope)
            ORDER BY CASE WHEN scope='global' THEN 0 ELSE 1 END, id DESC LIMIT 1
        """), {"word": word, "user_scope": f"user:{int(user_id)}"}).mappings().first()
    return str(row["melimi"]).strip() if row else None
