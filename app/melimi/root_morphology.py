...
def _candidate_strips(surface,suffixes,kind):
    for suffix in suffixes:
        if surface.endswith(suffix) and len(surface)>len(suffix)+1: yield surface[:-len(suffix)],suffix,kind
...
def reduce_to_root(word,roots=None)->MorphologicalForm:
    ...
    def search(current,operations,depth):
        ...
        candidates=list(_candidate_strips(current,GRAMMATICAL_SUFFIXES,"grammar"))+list(_candidate_strips(current,DERIVATIONAL_SUFFIXES,"derivation")); candidates.sort(key=lambda x:(-len(x[1]),x[0]))
    ...
    root,ops=found; return MorphologicalForm(surface,root,tuple(s for _,s in ops),tuple(k for k,_ in ops))
