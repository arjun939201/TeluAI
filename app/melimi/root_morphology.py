...
def _adjectival_candidate(surface,roots):
    ...
    if surface.endswith("ంగా"):
        candidate=surface[:-3]+"ం"
        if candidate in roots:return candidate,"ంగా","adjective_predicate"
    if surface.endswith("గా"):
        candidate=surface[:-2]+"ం"
        if candidate in roots:return candidate,"గా","adjective_predicate"
    if surface+"ం" in roots:return surface+"ం","BARE_AM","bare_nominal"
...
def reduce_to_root(word,roots=None):
    ...
    for finder in (_case_candidate,_plural_candidate,_verb_candidate,_derived_voice_candidate,_adjectival_candidate):
        result=finder(surface,roots)
        if result:
            if finder is _case_candidate:
                root,ops=result; return MorphologicalForm(surface,root,tuple(x[1] for x in ops),tuple(x[0] for x in ops))
            root,suffix,kind=result; return MorphologicalForm(surface,root,(suffix,),(kind,))
...
def apply_operation(root,kind,suffix):
    ...
    if kind=="case":
        if suffix==ACCUSATIVE:
            if root.endswith("లు"):return root[:-1]+"ను"
            if root.endswith("ం"):return root[:-1]+"ాన్ని"
            if root.endswith("ా"):return root+"ను"
            if root.endswith("ు"):return root+"ను"
            return root+"ని"
        if suffix==DATIVE:
            if root.endswith("లు"):return root[:-1]+"కు"
            if root.endswith("ం"):return root[:-1]+"ానికి"
            if root.endswith("ా"):return root+"కు"
            if root.endswith("ు") or root.endswith("ి"):return root+"కు"
            return root+"కి"
        if suffix==INSTRUMENTAL:return root[:-1]+"తో" if root.endswith("లు") else root+"తో"
        if suffix==LOCATIVE:return root[:-1]+"లో" if root.endswith("లు") else root+"లో"
        if suffix==OBLIQUE:
