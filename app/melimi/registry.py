...
FUNCTION_WORDS={
"నేను","నాకు","నా","నువ్వు","నీ","నీకు","మీరు","మీకు","అతను","ఆమె","వారు","ఇది","అది","ఇవి","అవి",
"ఏమి","ఏం","ఏంటి","ఎలా","ఎందుకు","ఎక్కడ","ఎప్పుడు","ఎవరు","ఎంత","ఎన్ని","ఒక","మరియు","లేదా","కానీ",
"అయితే","కూడా","మాత్రం","ఇంకా","ఇప్పుడు","అప్పుడు","ఇక్కడ","అక్కడ","లో","కు","కి","తో","నుండి","నుంచి",
"పై","కింద","కోసం","వల్ల","గురించి","మధ్య","లా","గా","అని","అంటే","లేదు","లేను","లేవు","కాదు","వద్దు",
"ఉంది","ఉన్న","ఉన్నారు","ఉన్నాను","ఉన్నావు","చెప్పు","చెప్పండి","రా","రండి","వెళ్లు","వెళ్లి","చేయి","చేయండి",
"అవును","సరే","హా","హాయ్"
}
...
def lexical_inventory():
    ...
    try:
        ...
        for std,mel in roots.items():
            standard_to_melimi[std]=mel; melimi_to_standard[mel]=std; registered.add(mel); forbidden.add(std)
...
def reload_registry(): lexical_inventory.cache_clear(); load_root_dictionary.cache_clear()
...
def strict_violations(text):
    ...
    for w in tokenize(text):
        if w in inv["standard_to_melimi"]: violations.append({"standard":w,"melimi":inv["standard_to_melimi"][w]})
    return violations

def analyze_word(word):
    inv=lexical_inventory(); form=reduce_to_root(word,load_root_dictionary()); mel=inv["standard_to_melimi"].get(form.root,"")
    transformed=reapply_operations(mel,form) if mel else ""
