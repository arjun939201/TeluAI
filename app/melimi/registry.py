...
def _build_lexical_inventory(version: int):
    ...
    try:
        ...
        for std, mel in roots.items():
            standard_to_melimi[std] = mel
            melimi_to_standard[mel] = std
            registered.add(mel)
            forbidden.add(std)
    except Exception:
        pass
    try:
        ...
        for d in build_index(version):
            for e in d.entries:
                ...
                if status in {"native", "native_telugu", "established", "corpus-supported", "derived-by-rule"} or st in {"native", "native_telugu"}:
                    native.update(mel)
                    native.update(std)
                if status in {"loan", "loanword", "borrowed", "foreign"} or st in {"loan", "loanword", "borrowed", "foreign"}:
                    loan.update(std)
    except Exception:
        pass
...
def reload_registry():
    _lexical_inventory_for_version.cache_clear()
...
def lookup_word(word: str, version: int | None = None) -> dict:
    ...
    melimi_source = inv["melimi_to_standard"].get(clean, "")
    return {
        "word": word,
        "normalized": clean,
        "registered": clean in inv["registered"],
        "native": clean in inv["native"],
        "loan": clean in inv["loan"],
        "is_melimi": clean in inv["melimi_to_standard"] or clean in inv["native"],
        "standard_equivalent": standard_target,
        "melimi_equivalent": standard_target,
        "standard_source": melimi_source,
        "exact_match": bool(standard_target or melimi_source or clean in inv["registered"]),
        "authority": "MASTER" if clean in inv["registered"] else "UNKNOWN",
    }
...
def strict_violations(text):
    inv = lexical_inventory()
    return [{"standard": w, "melimi": inv["standard_to_melimi"][w]} for w in tokenize(text) if w in inv["standard_to_melimi"]]


def analyze_word(word):
    inv = lexical_inventory()
    clean = _clean_word(word)
    lexical = lookup_word(clean)
    roots = load_root_dictionary()
    analysis = analyze_surface(clean, roots)
    ...
    return {
        "word": word,
        "normalized": clean,
        "registered": lexical["registered"],
        "native": lexical["native"],
        "loan": lexical["loan"],
        "lexical": lexical,
        "melimi_equivalent": target,
