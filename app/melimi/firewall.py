...
def _subject_lexicon_for_version(version:int):
    ...
    for source,melimi in load_root_dictionary(version).items():
        forbidden[source]=melimi; preferred[source]=melimi; registered.add(melimi)
    try:
        for doc in build_index(version):
...
def subject_lexicon(version:int|None=None):
    ...
    return _subject_lexicon_for_version(int(version))

def reload_firewall(): _subject_lexicon_for_version.cache_clear()

def _match_root(token,forbidden,adjective_capable=None):
    if not token:return None
    form=reduce_to_root(token,forbidden)
    if form.root in forbidden:
        melimi_root=forbidden[form.root]; capable=(form.root,melimi_root) in (adjective_capable or set())
        if capable and is_non_am_ending_melimi(melimi_root):
            if form.kinds==("adjective",): return form.root,"",melimi_root
            if form.kinds==("adjective_predicate",): return form.root,"గా",reapply_operations(melimi_root,form)
        return form.root,form.suffixes[-1] if form.suffixes else "",reapply_operations(melimi_root,form)
