...
def _lookup_standard(word: str, roots: dict[str,str]):
    ...
    if re.search(r"[\u0C00-\u0C7F]",word):
        ...
        for key,value in roots.items():
            ...
            if ratio>=0.90 and (best is None or ratio>best[0]): best=(ratio,key,value)
...
def _lookup_english_authoritative(query: str):
    ...
    if not q or re.search(r"[\u0C00-\u0C7F]",q): return None
    with SessionLocal() as db:
        rows=db.scalars(select(KnowledgeEntry).where(KnowledgeEntry.status=="MASTER").order_by(KnowledgeEntry.id.desc()).limit(5000)).all()
        for row in rows:
            try: metadata=json.loads(row.metadata_json or "{}")
            except (TypeError,ValueError): metadata={}
            candidates=[metadata.get(k) for k in ("english","gloss","meaning","definition","standard","source")]+[row.key]
            if any(isinstance(v,str) and re.sub(r"\s+"," ",v).casefold()==q for v in candidates):
                target=str(row.value or metadata.get("melimi") or "").strip()
                if target and re.search(r"[\u0C00-\u0C7F]",target) and not re.search(r"[A-Za-z]",target): return target
    return None


def _extract_lookup_word(q: str):
    patterns=(
        r"^(.+?)\s+మేలిమి(?:-?తెలుగు)?(?:లో|లోని)?\s+(?:ఏమంటారు|ఏమిటి|ఏంటి)\??$",
        r"^(.+?)\s+మేలిమి\s+తెలుగులో\s+(?:ఏమంటారు|ఏమిటి|ఏంటి)\??$",
        r"^మేలిమి\s+తెలుగులో\s+(.+?)\s+(?:ఏమంటారు|ఏమిటి|ఏంటి)\??$",
        r"^(.+?)\s+అంటే\s+ఏమిటి\??$",
        r"^(.+?)\s+అర్థం\s+ఏమిటి\??$",
        r"^(.+?)\s+(?:ఏంటి|ఏమిటి)\??$",
    ...
    for pattern in patterns:
        ...
        if match:
            word=re.sub(r"^మేలిమి\s+తెలుగులో\s+", "", match.group(1).strip()).strip(" ?.!,:;")
            if word: return word
    return None
...
def _lookup_content(q: str):
    ...
    with SessionLocal() as db:
        ...
        for row in rows:
            if row.kind in {"CONTENT","EXAMPLE","POST","FACT","NOTE"} and (row.value or "").strip().casefold()==q.casefold(): candidates.append((row.value.strip(),row.metadata_json or "{}"))
        examples=db.scalars(select(MelimiExample).where(MelimiExample.status=="MASTER")).all()
        for row in examples:
            if (row.melimi_text or "").strip().casefold()==q.casefold(): candidates.append((row.melimi_text,row.standard_text))
    if not candidates: return None
    value,meta=candidates[0]
    try: meaning=str(json.loads(meta or "{}").get("meaning","")).strip() if str(meta).strip().startswith("{") else str(meta).strip()
    except Exception: meaning=""
    return f"{value}\n{meaning}" if meaning else value


def _grammatical_role(form):
    roles=[]
    for kind,suffix in form.operations:
        if kind=="case": roles.append({"ACCUSATIVE":"కర్మ విభక్తి (Accusative)","DATIVE":"సంప్రదాన విభక్తి (Dative)"}.get(suffix,suffix))
        elif kind=="grammar": roles.append({"లు":"బహువచనం","ల":"బహువచనం","లను":"బహువచనం + కర్మ విభక్తి","లని":"బహువచనం + కర్మ విభక్తి","లకు":"బహువచనం + సంప్రదాన విభక్తి","లకై":"బహువచనం + కొరకు రూపం","లపై":"బహువచనం + పై విభక్తి","లతో":"బహువచనం + తో విభక్తి","లలో":"బహువచనం + లో విభక్తి","ను":"కర్మ విభక్తి","ని":"కర్మ విభక్తి","కు":"సంప్రదాన విభక్తి","కి":"సంప్రదాన విభక్తి","లో":"స్థాన విభక్తి","తో":"సహచర్య విభక్తి","పై":"స్థాన/పై విభక్తి","గా":"రీతి రూపం"}.get(suffix,suffix))
        elif kind=="derivation": roles.append(f"-{suffix} ప్రత్యయ రూపం")
        elif kind.startswith("adjective"): roles.append("విశేషణ రూపం")
    return " + ".join(dict.fromkeys(roles)) if roles else None


def _lookup_inflected(word,roots):
    form=reduce_to_root(word,roots)
    if form.root not in roots or not form.operations: return None
    melimi=reapply_operations(roots[form.root],form); role=_grammatical_role(form)
    return f"{melimi}\nవ్యాకరణ పాత్ర: {role}" if role else melimi


def answer(message: str, mode: str):
    if mode!="melimi": return None
    q=re.sub(r"\s+"," ",(message or "").strip())
    if q in {"మేలిమి తెలుగు అంటే ఏమిటి?","మేలిమి తెలుగు అంటే ఏమిటి"}:
        return "మేలిమి తెలుగు అనేది తెలుగు ఆధారిత వేఱైన నుడి రూపం. ఇందులో కుదిరిన మేలిమి మాటలు, పదనిర్మాణ నియమాలు, పదార్థభేదాలు, వాడుకరీతులు ఉంటాయి."
    word=_extract_lookup_word(q)
    if word:
        roots=load_root_dictionary(); inflected=_lookup_inflected(word,roots)
        ...
        return None
    target=_lookup_english_authoritative(q)
    if target: return target
    return _lookup_content(q)


async def try_deterministic_answer(message: str, mode: str, history_count: int=0):
    if history_count: return None
