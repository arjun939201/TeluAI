...
def seed_language_space_for_tests():
    ...
    with SessionLocal() as db:
        for standard,melimi in ROOTS.items():
            ...
            if row is None: db.add(MelimiRoot(standard_root=standard,melimi_root=melimi,status="MASTER",source="test-fixture"))
            else: row.melimi_root=melimi; row.status="MASTER"
        for path,kind,entries in DOCUMENTS:
            row=db.query(MelimiDocument).filter(MelimiDocument.path==path).first()
            if row is None: db.add(MelimiDocument(path=path,kind=kind,text=json.dumps(entries,ensure_ascii=False) if entries else f"Test {kind} Language Space document",entries_json=json.dumps(entries,ensure_ascii=False),status="MASTER",source="test-fixture"))
        db.commit()
    ...
    from app.retrieval.knowledge import reload_vocabulary
    reload_root_dictionary(); reload_index(); reload_registry(); reload_firewall(); reload_vocabulary()
    yield
