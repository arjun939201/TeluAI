...
function newChat(){history=[];currentConversationId=null;chatContainer.querySelectorAll(".message,.error-message,.typing-message").forEach(x=>x.remove());if(welcome)welcome.style.display="";messageInput.value="";resizeInput();messageInput.focus()}
clearButton.addEventListener("click",newChat);mobileNewChat.addEventListener("click",newChat);
...
async function loadConversations(){try{const r=await fetch("/conversations",{credentials:"same-origin"});if(!r.ok)return;const d=await r.json();const box=document.getElementById("chatHistory");box.innerHTML="";(d.conversations||[]).forEach(c=>{const item=document.createElement("div");item.className="chat-item"+(c.id===currentConversationId?" active":"");item.innerHTML=`<span class="chat-icon">◌</span><span>${escapeHtml(c.title||"New chat")}</span>`;item.addEventListener("click",()=>loadConversation(c.id));box.appendChild(item)})}catch(e){}}
async function loadConversation(id){try{const r=await fetch(`/conversations/${encodeURIComponent(id)}`,{credentials:"same-origin"});if(!r.ok)return;const d=await r.json();currentConversationId=id;history=[];chatContainer.querySelectorAll(".message,.error-message,.typing-message").forEach(x=>x.remove());if(welcome)welcome.style.display="none";(d.messages||[]).forEach(m=>{addMessage(m.content,m.role,m.role==="assistant"&&mode==="melimi",[]);history.push({role:m.role,content:m.content})});loadConversations()}catch(e){addError("Could not load this conversation.")}}
